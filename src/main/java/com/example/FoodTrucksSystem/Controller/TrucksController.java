...
public class TrucksController {
    ...
    @PostMapping("/add")
    public ResponseEntity<API> addPaymentType(@RequestBody @Valid Trucks mytrk){
    ...
    ResponseEntity<API> editTruck(@PathVariable Long truckid,@RequestBody Trucks trck) {
        ...
        if (result.isPresent())
            return ResponseEntity.status(HttpStatus.OK).body(new API("Edit PaymentType "+truckid+" for : "+result.toString(),200));
        else
